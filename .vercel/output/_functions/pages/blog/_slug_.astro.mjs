...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const applyInlineMarkdown = (value) => value.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>");
  const renderMarkdown = (value) => {
    ...
    return blocks.map((block) => {
      ...
      if (escaped.startsWith("# ")) {
        return `<h2>${applyInlineMarkdown(escaped.slice(2))}</h2>`;
      }
      const paragraph = applyInlineMarkdown(escaped).replace(/\n/g, "<br />");
